...
namespace SnowyBot.Modules
{
	public class LavalinkModule : ModuleBase
	{
		...
		public async Task Play([Remainder] string query = null)
		{
			...
			Guild guild = await guilds.GetGuild(Context.Guild.Id).ConfigureAwait(false);
			List<PlaylistVideo> playlist = PlaylistId.TryParse(query) != null ? await DiscordService.youTube.Playlists.GetVideosAsync(PlaylistId.Parse(query)).ToListAsync().ConfigureAwait(false) : null;
			...
			if (playlist != null)
			{
				await ProcessPlaylist(player, playlist, !playing);
		...
		public async Task ProcessPlaylist(LavaPlayer player, List<PlaylistVideo> playlist, bool playFirst)
		{
			...
			ParallelLoopResult result = Parallel.For(0, playlist.Count, (int num) =>
			 {
				 ...
				 if (track != null)
				 {
					 LoggingService.LogAsync("PRLL", LogSeverity.Info, num.ToString());
					 if (num == 0 && playFirst)
						 Play(track);
					 else
			...
			if (result.IsCompleted)
			{
				...
				for (int i = 0; i < sortingList.Count; i++)
				{
					...
					LoggingService.LogAsync("LIST", LogSeverity.Info, sortingList.ElementAt(i).index.ToString() + " " + sortingList.ElementAt(i).track.Title + "\n");
				}
			}
		}
		public static void ProcessTimestamp(ref string query, out int seconds)
		{
			if (query != null)
			{
